...
class RDBService:
    ...
    @classmethod
    def get_by_userid(cls, db_schema, table_name, column_name, userid):
    ...
    @classmethod
    def find_by_template(cls, db_schema, table_name):
        # , template, field_list
    ...
    @classmethod
    def get_next_id(cls, db_schema, table_name):

        conn = RDBService._get_db_connection()
        cur = conn.cursor()

        sql = "select max(ID) from " + db_schema + "." + table_name
        print("SQL Statement = " + cur.mogrify(sql, None))

        res = cur.execute(sql)
        res = cur.fetchall()
        res = [x['max(ID)'] for x in res][0]
        res = res + 1
